<x-vendor-dashboard-layout>
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Daftar Layanan Saya</h2>
    ...
    <div class="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select id="service-type-filter" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary">
                ...
                <div class="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow p-6">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                              {{ $service->pivot->is_available ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' }}">
                            {{ $service->pivot->is_available ? 'Tersedia' : 'Tidak Tersedia' }}
                        </span>
                    </div>
                    ...
                    <div class="mb-4">
                        <p class="text-lg font-bold text-primary">
                            Rp {{ number_format($service->pivot->price ?: $service->price, 0, ',', '.') }}
                        </p>
                    ...
                    <div class="flex justify-between items-center">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100">
                            {{ $service->category ?: $service->serviceType?->name ?: 'Umum' }}
                        </span>
                        
                        <div class="flex space-x-2">
                            <a href="{{ route('vendor.services.edit', $service->pivot->id) }}" 
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Anda belum menambahkan layanan. Klik tombol di atas untuk menambahkan layanan baru.
            </p>
        </div>
    @endif
</x-vendor-dashboard-layout>