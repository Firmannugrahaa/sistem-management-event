<x-app-layout>
                ...
                <form action="{{ route('event-packages.update', $eventPackage->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="package_price" id="package_price" value="{{ old('package_price', $eventPackage->package_price) }}"
                    ...
                    <div>
                        ...
                        @if($eventPackage->thumbnail_path)
                            <div class="mb-3">
                                <img src="{{ Storage::url($eventPackage->thumbnail_path) }}" alt="Current Thumbnail" class="h-32 w-auto rounded-lg object-cover border">
                            </div>
                        @endif
                        ...
                        <p class="mt-1 text-sm text-gray-500">Format: JPG, PNG, WEBP. Max: 2MB. Biarkan kosong jika tidak ingin mengubah.</p>
                        @error('thumbnail')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div id="items-container" class="space-y-3">
                            @forelse($eventPackage->items as $index => $item)
                                <div class="item-row flex gap-3">
                                    <select name="items[{{ $index }}][vendor_product_id]" 
                                        ...
                                        @foreach($vendorProducts as $product)
                                            <option value="{{ $product->id }}" {{ $item->vendor_product_id == $product->id ? 'selected' : '' }}>
    ...
    @push('scripts')
    <script>
        let itemIndex = {{ $eventPackage->items->count() }};
