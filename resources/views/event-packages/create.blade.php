<x-app-layout>
    ...
    <div class="py-8" x-data="eventPackageForm({{ Js::from($vendorCatalogItems) }}, {{ Js::from($vendorPackages) }})">
                ...
                <form action="{{ route('event-packages.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <select name="event_type" id="event_type" required
                            ...
                            <option value="Wedding" {{ old('event_type') == 'Wedding' ? 'selected' : '' }}>Wedding (Pernikahan)</option>
                            <option value="Prewedding" {{ old('event_type') == 'Prewedding' ? 'selected' : '' }}>Prewedding</option>
                            <option value="Birthday" {{ old('event_type') == 'Birthday' ? 'selected' : '' }}>Birthday (Ulang Tahun)</option>
                            ...
                            <option value="Engagement" {{ old('event_type') == 'Engagement' ? 'selected' : '' }}>Engagement (Tunangan)</option>
                            <option value="Other" {{ old('event_type') == 'Other' ? 'selected' : '' }}>Other (Lainnya)</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="pricing_method" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#012A4A] focus:border-transparent">
                                <option value="manual">Manual (Base Price)</option>
                                <option value="auto">Otomatis (Sum of Items)</option>
                                <option value="hybrid">Hybrid (Base + Items?)</option>
                            </select>
                        </div>
                    </div>
    ...
    @push('scripts')
    <script>
        function eventPackageForm(catalogItemsData, packagesData) {
            return {
                showModal: false,
                activeTab: 'products',
                searchQuery: '',
                ...
                confirmSelection() {
                    ...
                    this.tempSelectedCatalogItems.forEach(id => {
                        ...
                        if (item) {
                            this.addIfNotExist('product', item); // Use 'product' as type key mainly because backend expects 'product' or 'package' enum
                        }
                    });
