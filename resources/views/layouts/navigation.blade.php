    ...
    <div class="flex flex-col h-full">
        ...
        <div class="flex-1 flex flex-col justify-between py-6 px-3 overflow-y-auto">
            <div class="space-y-2">
                ...
                @php
                    $dashboardRoute = auth()->user()->hasRole('Vendor') ? route('vendor.dashboard') : route('dashboard');
                    $isDashboardActive = request()->routeIs('dashboard') || request()->routeIs('vendor.dashboard');
                @endphp
                <a href="{{ $dashboardRoute }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-[#F0F0F0] {{ $isDashboardActive ? 'bg-[#012A4A] text-white' : 'text-[#1A1A1A]' }}">
                    <svg class="w-6 h-6 flex-shrink-0 p-1 rounded {{ $isDashboardActive ? 'bg-[#c1dfeb] text-[#012A4A]' : 'bg-[#012A4A] text-white' }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                {{-- Portfolio / Galeri (Company) --}}
                @if(auth()->user()->hasRole('Owner') || auth()->user()->hasRole('SuperUser') || auth()->user()->hasRole('Admin'))
                <a href="{{ route('company.portfolios.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-[#F0F0F0] {{ request()->routeIs('company.portfolios.*') ? 'bg-[#012A4A] text-white' : 'text-[#1A1A1A]' }}">
                    <svg class="w-6 h-6 flex-shrink-0 p-1 rounded {{ request()->routeIs('company.portfolios.*') ? 'bg-[#c1dfeb] text-[#012A4A]' : 'bg-[#012A4A] text-white' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
             ...
             @endif
                ...
                @else
                @if(auth()->user()->hasRole('Owner') || auth()->user()->hasRole('Vendor') || auth()->user()->hasRole('Admin'))
                <a href="{{ route('events.index') }}"
                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-[#F0F0F0] text-[#1A1A1A]">
                    <svg class="w-6 h-6 flex-shrink-0 p-1 rounded bg-[#012A4A] text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M8 2v4" />
                ...
                {{-- My Invoices - Hidden for SuperUser and Staff --}}
                @unlessrole('SuperUser|Staff')
