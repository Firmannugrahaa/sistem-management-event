    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-end mb-6">
                <a href="{{ route((request()->routeIs('company.*') ? 'company' : 'vendor') . '.products.create') }}" 
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route((request()->routeIs('company.*') ? 'company' : 'vendor') . '.products.edit', $product->id) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                                            <form action="{{ route((request()->routeIs('company.*') ? 'company' : 'vendor') . '.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus layanan ini?');" class="inline">
                                                @csrf
                                                @method('DELETE')
